...
class EvyukaFormUpdater:
    ...
    def process_form(self, form_path: Path, create_backup=True):
        ...
        if not (form_path.name.startswith('evyuka_form_') and form_path.name.endswith('.xml')):
            logger.error(
                f"SECURITY: Refusing to process non-evyuka form file: {form_path.name}")
            return False
            ...
            if is_evyuka:
                ...
                if validation_elem is not None:
                    ...
                    if hint_text:  # Only if hint exists
                        ...
                        logger.warning(
                            f"No hint element found for {field_name}, cannot add validation-group")

        if validation_updates:
            # Apply only validation group changes
            self.save_file_with_validation_groups_only(
                form_path, original_content, validation_updates)
            logger.info(
                f"Updated {len(validation_updates)} validation groups in {form_path.name}")
        else:
    ...
    def save_file_with_validation_groups_only(self, file_path, original_content, validation_updates):
        ...
        for update in validation_updates:
            ...
            if action == 'add':
                ...
                logger.debug(
                    f"Added validation-group '{validation_group}' to {field_name}")
                ...
                updated_content = re.sub(pattern, replacement, updated_content, count=1)
                logger.debug(
                    f"Updated validation-group for {field_name}: '{old_group}' -> '{validation_group}'")
                ...
                for field in root.findall('.//field'):
                    ...
                    if is_evyuka:
                        ...
                        if validation_elem is None:
                            form_errors.append(
                                f"Missing validation-group for {field_name}")
                        elif validation_elem.text != expected_group:
                            form_errors.append(
                                f"Incorrect validation-group for {field_name}: got '{validation_elem.text}', expected '{expected_group}'")

                if form_errors:
                    validation_errors.append(
                        f"{form_path.name}: {', '.join(form_errors)}")
                else:
