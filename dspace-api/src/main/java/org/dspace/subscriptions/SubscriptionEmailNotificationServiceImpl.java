        ...
        try {
            ...
            if (subscriptionType2generators.keySet().contains(subscriptionType)) {
                ...
                for (Subscription subscription : subscriptions) {
                    ...
                    // Set the current user to the eperson who has subscribed because the Solr query checks
                    // the permissions of the current user in the ANONYMOUS group. And if there is no user - the
                    // `current user = null` it will send email with no new Items.
