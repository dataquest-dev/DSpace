...
class Validator(object):
    ...
    :param timeout:
    Optional timeout in seconds for all requests to the server.
    """
    ...
    def indexed_in_BASE(self):
        ...
        result = urllib.request.urlopen(LOOKUP_URL + data)
        ...
        if indexed.text == 'True':
            ...
            if collection:
                message += """ <br/><a href="http://www.base-search.net/Search/Results?q=dccoll:%s"
                >See all documents of this repository in BASE</a>""" % collection
        else:
    ...
    def incremental_harvesting(self, verb, granularity, metadataPrefix='oai_dc', sample_size=50):
        ...
        try:
            ...
            message = "Incremental harvesting (%s granularity) of %s could not be checked: %s" % (granularity, verb, str(exc))
    ...
    def minimal_dc_elements(self, minimal_set=MINIMAL_DC_SET, sample_size=50):
        ...
        try:
            ...
            message = 'Minimal DC elements could not be checked: %s' % str(
                exc)
    ...
    def check_resumption_token(self, verb, metadataPrefix='oai_dc'):
        ...
        try:
            ...
            message = 'Resumption requests could not be checked: %s' % str(exc)
        ...
        try:
            ...
            message = 'Error during resumption request: %s' % str(exc)
            self.results['ResumptionToken'] = ('error', message)
    ...
    def check_deleting_strategy(self):
        ...
        try:
            ...
            message = "Deleting strategy could not be checked: %s" % str(
                exc)
            self.results['DeletingStrategy'] = ('unverified', message)
            return
        if deleting_strategy == 'no':
            message = (u'Deleting strategy is "no" – ' +
                       'recommended is persistent or '
                       'transient.')
    ...
    def check_handle(self):
        ...
        if "123456789" in sample_handle:
            ...
            return
        try:
            resp = urllib.request.urlopen(sample_handle).read()
