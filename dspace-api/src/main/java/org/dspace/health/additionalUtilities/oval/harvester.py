...
def configure_record_iterator(
    base_url, protocol_version, HTTPmethod, timeout=None
):
    """Class factory for record iterators.
       ...
       :param timeout: Optional timeout for the HTTP requests
       sent to the server.
    """
    class RecordIterator(object):
        ...
        """
        def __init__(
            self, verb, metadataPrefix, _from=None, until=None, deleted=False
        ):
        ...
        def _is_not_deleted(self, record):
            if self.element == 'record':
                ...
                # work on header element directly in case of ListId
                header = record
