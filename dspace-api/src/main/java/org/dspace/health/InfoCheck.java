...
public class InfoCheck extends Check {
    ...
    public String run(ReportInfo ri) {
        ...
        String fromTill = "From - Till: "
            + ri.from().format(DateFormatConstants.DATE_FORMATTER)
            + " - " + ri.till().format(DateFormatConstants.DATE_FORMATTER);
        ...
        // Build the directory stats array. report-diff-fields.json addresses these positionally
        // (directoryStats/0 = assetstore, directoryStats/1 = log dir), so an entry is appended for
        // every directory even when it cannot be read - dropping one would shift the other's index.
