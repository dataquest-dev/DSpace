...
public class HealthReportIT extends AbstractIntegrationTestWithDatabase {
    ...
    /** Name of the license check, as registered in {@code config/modules/healthcheck.cfg}. */
    private static final String LICENSE_CHECK = "License";
    /** Name of the general information check, as registered in {@code config/modules/healthcheck.cfg}. */
    private static final String INFO_CHECK = "General Information";
    /** Name of the item check, as registered in {@code config/modules/healthcheck.cfg}. */
    private static final String ITEM_CHECK = "Item summary";
    /** Name of the metadata check, as registered in {@code config/modules/healthcheck.cfg}. */
    private static final String METADATA_CHECK = "Metadata check";

    /**
     * The {@code -c} option addresses a check by its position in {@code healthcheck.checks}, so a test that
     * hard-codes the number silently runs a different check whenever that list is reordered. Resolve the
     * position from the check name instead.
     *
     * @param checkName name of the check, as configured in {@code healthcheck.checks}
     * @return the value to pass to {@code -c}
     */
    private static String checkIndex(String checkName) {
        for (int i = 0; i < HealthReport.getNumberOfChecks(); i++) {
            if (checkName.equals(HealthReport.getCheckName(i))) {
                return String.valueOf(i);
            }
        }
        throw new IllegalStateException("Check not configured in healthcheck.checks: " + checkName);
    }
    ...
    public void testLicenseCheck() throws Exception {
        ...
        String[] args = new String[] { "health-report", "-c", checkIndex(LICENSE_CHECK) };
        ScriptLauncher.handleScript(args, ScriptLauncher.getConfig(kernelImpl), testDSpaceRunnableHandler, kernelImpl);
    ...
    public void testMultipleChecks() throws Exception {
        TestDSpaceRunnableHandler handler = new TestDSpaceRunnableHandler();
        // Run only the General Information and the License check: space-separated
        String[] args =
                new String[] { "health-report", "-c", checkIndex(INFO_CHECK), checkIndex(LICENSE_CHECK) };
        ScriptLauncher.handleScript(args, ScriptLauncher.getConfig(kernelImpl), handler, kernelImpl);

        assertThat(handler.getErrorMessages(), empty());
        List<String> messages = handler.getInfoMessages();
        assertThat(messages, hasItem(containsString("HEALTH REPORT ")));
        assertThat(messages, hasItem(containsString(INFO_CHECK + ":")));
        assertThat(messages, hasItem(containsString(LICENSE_CHECK + ":")));
        // the check that was not selected must NOT be present
        boolean hasItemSummary = messages.stream().anyMatch(m -> m.contains(ITEM_CHECK + ":"));
    ...
    public void testMetadataCheck() throws Exception {
        ...
        // Only the Metadata check runs. The -c option addresses a check by its position in
        // healthcheck.checks, which is ordered differently here than on dtq-dev, so resolve
        // the position from the check name.
